"""v2.5 voice-трек, часть B: speaker_id unknown + маршрутизация в main.py."""
import re
import py_compile
...
if cnt == 1:
    src = src.replace('а честно остаётся "default"',
                      'а честно получает метку "unknown" (безопасный режим)')
    report.append("✅ speaker_id: docstring обновлён")
else:
    report.append("⚠️ speaker_id: docstring-фраза не найдена (не критично)")
...
# сигнатура conversation_mode (РАНЬШЕ замены вызова, чтобы вызов стал уникальным)
src, n7 = re.subn(
...
if src.count("conversation_mode(cfg, recorder, stt, llm, tts, log, memory, personality)") == 1:
    ...
    report.append("❌ main: вызов conversation_mode не уникален/не найден")
...
# ================= компиляция =================
for rel in ["main.py", "core/speaker_id.py", "core/kids_pipeline.py"]:
    try:
